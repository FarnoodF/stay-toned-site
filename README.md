...
- `/science/` — science and accuracy report (target formulas, per-ingredient math, AI estimate limits)
...
- `/delete-account/` — public Google Play account-deletion instructions and data-retention details
- `/offer/` — promotional-offer app opener with App Store/Google Play fallback and explicit post-install rescan instructions

Promotional offer tokens live in the URL fragment (`#token=...`), which is not sent to the static host in an HTTP request. The offer page is excluded from search indexing and never displays or persists the token.
...
Remove any URL-forwarding rule for the same host so GitHub Pages can serve `/`, `/policy/`, `/terms/`, `/contact/`, and `/delete-account/`.
...
Use `https://staytoned.farnood.tech/delete-account/` as the Google Play account-deletion URL.
